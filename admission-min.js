...
function getSuccessTemplateValues(){
    let fetchUrl;
    if(env === 'PRE-PROD'){
        fetchUrl = `https://dev-api.simpo.ai/crm/templates?businessId=${bId}&status=true&formCategory=ADMISSION`
    }
    else if(env === 'PROD'){
        fetchUrl = `https://api.simpo.ai/crm/templates?businessId=${bId}&status=true&formCategory=ADMISSION`
    }
    fetch(fetchUrl)
    .then(response => {
        if (!response.ok) {
...
async function fetchAllRegistrationFields() {
    let fetchUrl;
    if (env === 'PRE-PROD') {
        fetchUrl = `https://dev-api.simpo.ai/crm/fields?businessId=${bId}&showInForm=ADMISSION&isPagination=false&fieldCategory=ADMISSION`
    }
    else if (env === 'PROD') {
        fetchUrl = `https://api.simpo.ai/crm/fields?businessId=${bId}&showInForm=ADMISSION&isPagination=false&fieldCategory=ADMISSION`
    }
    fetch(fetchUrl)
        .then(response => {
            if (!response.ok) {
...
function loadAllRegistrationFields() {
    ...
    const registrationForm = document.getElementById("complete-admission-form");
...
async function submit() {
    ...
    if(env === 'PRE-PROD'){
        submitFormEndPoint = `https://dev-api.simpo.ai/crm/registration/admission?registrationId=${payload?.registrationId || ''}`
    }
    else if(env === 'PROD'){
        submitFormEndPoint = `https://api.simpo.ai/crm/registration/admission?registrationId=${payload?.registrationId || ''}`
    }
